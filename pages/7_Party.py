...
with tab_l:
    ...
    if lb_party.empty:
        ...
        # Use Strength if present; otherwise derive from Lead
        ...
        if s_series.dropna().empty or "WardName" not in lb_party.columns:
            ...
            s_df = pd.DataFrame(
                {"Strength": s_series, "WardName": lb_party["WardName"]}
            ).dropna(subset=["Strength"])

            # Keep logical order of categories
            ...
            # Split into Lead (positive bins; exclude "0") and Trail (negative bins)
            ...
            st.markdown("**Lead**")
            if lead_tbl.empty:
                st.caption("No categories with more than one ward.")
            else:
    ...
    st.subheader("🏅 Winning Candidates (Selected Party)")
    if lb_ward.empty or "Rank" not in lb_ward.columns:
        ...
        if winners.empty:
            ...
            styled_win = color_rows_uniform_party(winners_tbl, sel_party).format({
                "Votes": "{:,.0f}",
                "Lead": "{:,.0f}",
                "Vote share (%)": "{:,.2f}%"
            })
    ...
    st.subheader("📉 Losing Candidates (Selected Party)")
    if lb_ward.empty or "Rank" not in lb_ward.columns:
        st.info("No Rank data available.")
    else:
        keys = _ward_join_keys(lb_ward)
        ...
        if losers.empty:
            ...
            styled_lose = color_rows_uniform_party(losers_tbl, sel_party).format({
                "Votes": "{:,.0f}",
                "Trail": "{:,.0f}",
                "Vote share (%)": "{:,.2f}%"
            })
